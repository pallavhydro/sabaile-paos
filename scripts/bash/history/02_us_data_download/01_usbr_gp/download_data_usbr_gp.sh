...
for j in "${!stntypes[@]}"; do
   ...
   # add more conditional if Climate and/ or Snotel is to be included
   fi
done
...
for (( j = 0; j <  9; j++ )) ; do 
   ...
   if [[ ${stntyp[j]} == ${stntypes[0]} ]]; then # Reservoir
      ...
      for k in "${!resparams[@]}"; do 
         ...
         # Generate weblink
         weblink="https://www.usbr.gov/gp-bin/webarccsv.pl?parameter="${stnid[j]}"%20"${resparams[k]}"&syer=${syear}&smnth=1&sdy=1&eyer=${eyear}&emnth=12&edy=31&format=2"
      ...
      for k in "${!strparams[@]}"; do 
         ...
         # Generate weblink
         weblink="https://www.usbr.gov/gp-bin/webarccsv.pl?parameter="${stnid[j]}"%20"${strparams[k]}"&syer=${syear}&smnth=1&sdy=1&eyer=${eyear}&emnth=12&edy=31&format=2"
