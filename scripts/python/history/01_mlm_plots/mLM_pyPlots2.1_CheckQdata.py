...
#---------------
# Lake level
flevel = "/Users/shresthp/pallav/01_work/Python/mLM_pyPlots/Observations/40100000.txt"
# Lake outflow
foutflow = "/Users/shresthp/pallav/01_work/Python/mLM_pyPlots/Observations/40330000.txt" 
# Lake spill
fspill = "/Users/shresthp/pallav/01_work/Python/mLM_pyPlots/Observations/40850000.txt"
# Output 1
fname1 = "/Users/shresthp/pallav/01_work/Python/mLM_pyPlots/09_Introducing_SeasonalPoolLevels/daily_waterBalance_lake1_003_optimizationTest1.bal"
# Output 2
...
#    plt.ylim(np.min([data1[:, iVar], data2[:, iVar]]), np.max([data1[:, iVar], data2[:, iVar]]))
    plt.ylim(np.min([data2[:, iVar]]), np.max([data2[:, iVar]]))
    
    # applying date tick labels to only bottom plots
    if (iPlot + ncols1) <= np.count_nonzero(order1):
        fig.set_xticklabels([])
        
#    plt.fill_between(date1, data1[:, iVar], color="lightseagreen", label='Constant Pool Levels', alpha = 0.5)
