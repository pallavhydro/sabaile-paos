...
# Output 1
fname1 = "/Users/shresthp/pallav/01_work/Python/mLM_pyPlots/13_PoolLevel_Revisit_afterMTT_anomalyInOpti/daily_waterBalance_lake1.bal"
...
#    plt.ylim(np.min([data1[:, iVar], dataSpill[:, iVar]]), np.max([data1[:, iVar], data2[:, iVar]]))
    ...
    plt.fill_between(date1, data1[:, iVar], color="lightseagreen", label='mHM', alpha = 0.5)
    ...
    plt.title(headers[iVar], **mssfont)
