...
#---------------
# Output 1
fname1 = "/Users/shresthp/pallav/01_work/Python/mLM_pyPlots/08_New_mLM_waterbalanceformat/daily_waterBalance_lake1_006_afterOptionalDataReadnAreaSpillOpti.bal"
# Output 2
...
#    plt.fill_between(date3, data3[:, iVar], color="gray", label='no Hydropower no Spill', alpha = 0.5)
    ...
    if (iVar == 23):
        plt.plot(date3, data3[:, 24], label='observed', color='black', alpha = 0.7)
    if (iVar == 24):
        plt.plot(date3, data3[:, 26], label='observed', color='black', alpha = 0.7)
