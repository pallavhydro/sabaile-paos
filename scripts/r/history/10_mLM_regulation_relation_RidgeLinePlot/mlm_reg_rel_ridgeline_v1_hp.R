...
for (i in 1:nLakes) {
  ...
  data_use_obs <- read.zoo(data[,-(4:5)], index.column = (1:3), format = "%Y %m %d") # construct zoo object using dates from the data itself
  ...
  data_use_obs_mon <- daily2monthly(data_use_obs, FUN=mean)
  data_qds_obs_mon <- daily2monthly(data_qds_obs, FUN=mean)
  
  
  ## Prepare data for plot
  # Combine monthly data
  data_mon <- cbind(data_tavg_ups_mon, data_use_obs_mon, data_qds_obs_mon) # ADD RULE CURVE DATA HERE
  ...
  color_tavg <- rev(heat.colors(10))
  color_hydropower <- brewer.pal(n = 8, name = "YlGnBu")
  ...
  xplot <- ggplot(data_mon, aes(x = `data_tavg_ups_mon`, 
                       y = month, 
                       fill = ..x..)) +
    geom_density_ridges_gradient(scale = 3, rel_min_height = 0.01) +
    scale_fill_gradientn(colours = color_tavg[-(1:2)]) +
  ...
  ##== Water elevation
  xplot <- ggplot(data_mon, aes(x = `data_use_obs_mon`, 
                                y = month, 
                                fill = ..x..)) +
    geom_density_ridges_gradient(scale = 3, rel_min_height = 0.01) +
    scale_fill_gradientn(colors = color_hydropower) +
    xlab("hydropower use \n (m3/s)") +
    labs( title = '') +
    scale_x_continuous(breaks = breaks_extended(5)) +
    scale_y_discrete(expand = c(0,2)) +
  ...
  xplot <- ggplot(data_mon, aes(x = `data_use_obs_mon`,
    ...
    xlab("demand curve for hydropower \n (m3/s)") +
    labs( title = '') +
    scale_x_continuous(limits = c(0,5), breaks = breaks_extended(5)) +
    scale_y_discrete(expand = c(0,2)) +
    theme(text=element_text(family = "Helvetica"),
          axis.ticks.length=unit(-0.25, "cm"),
...
# Output
setwd(opath)
pdf("regulation_hysteresis_ridgeline_demandcurve.pdf", width = 3*nColPlot, height = 6*nRowPlot) # each subplot is 5x8 inches
