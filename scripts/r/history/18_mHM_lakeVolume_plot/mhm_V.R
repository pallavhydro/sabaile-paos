...
# Set the basin name
bName = "Tres Marias"
subtitle_text = "dWL SSE optimized"

# Set the IO directory
iopath = "/Users/shresthp/tmp/eve_data_sawam/sawam/data/processed/mlmdevelopment/mlm_test_area/tests/i150_OFdeltas/03_output/OR_isolated/42_dwl_sse/"
...
main <- ggplot(data, aes(x=date)) + geom_line(aes(y=data[,obsCol], color="observation")) + # The color statement needs to be inside aes for the legend to appear
        ...
        theme(text=element_text(family = "Helvetica"), axis.ticks.length=unit(-0.25, "cm"), 
              ...
              panel.background = element_blank(), legend.position = c(0.95, 0.95),legend.justification = c(1, 0.8), legend.direction = "vertical",
              legend.background = element_rect(fill=alpha('white', 0)), plot.title = element_text(size = 16, face = "bold"), 
