...
setwd(ipath)
# abbrLakes = c("tma", "sob", "ita", "oro", "cas", "arg", "cor")
abbrLakes = c("tma", "sob", "tma", "oro", "tma", "tma", "tma")
# grandidLakes = c("2375", "2516", ...)
grandidLakes = c("2375", "2516", "2375", "2484", "2375", "2375", "2375")
nameLakes = c("Tr\u00E9s Marias", "Sobradinho", "Itaparica", "Or\u00F3s", "Castanh\u00E3o",
              "Armando Ribeiro Gon\u00E7alves", "Coremas")
...
for (i in 1:nLakes) {
  ...
  # Read observed water elevation (hws)
  ...
  # data_precip_ups_mon <- zoo(data[,3], data[,1])  # abstract tavg data
  ...
  data_mon <- cbind(data_precip_PET_ups_mon, data_hws_obs_mon)
  data_mon_clim <- zoo(t(monthlyfunction(data_mon, FUN = median))) # Climatology
  
  # Find out common time period overlap
  syear1 <- as.numeric(format(index(data_mon[first(which(!is.na(data_mon[,"data_precip_PET_ups_mon"])))]), "%Y"))
  syear2 <- as.numeric(format(index(data_mon[first(which(!is.na(data_mon[,"data_hws_obs_mon"])))]), "%Y"))
  eyear1 <- as.numeric(format(index(data_mon[last(which(!is.na(data_mon[,"data_precip_PET_ups_mon"])))]), "%Y"))
  eyear2 <- as.numeric(format(index(data_mon[last(which(!is.na(data_mon[,"data_hws_obs_mon"])))]), "%Y"))
  ...
  year_max_hws <- as.numeric(format(index(data_common[which.max(data_common[,"data_hws_obs_mon"]),1]), "%Y"))
  # year with minimum water elevation
  year_min_hws <- as.numeric(format(index(data_common[which.min(data_common[,"data_hws_obs_mon"]),1]), "%Y"))
  ...
  # data_mon_clim %>%
    xplot <- ggplot(data=data_mon_clim, aes(x=data_precip_PET_ups_mon, y=data_hws_obs_mon)) +
      # All monthly data
      geom_point(data = data_mon, aes(x=data_precip_PET_ups_mon, y=data_hws_obs_mon),
                   color="black",
                   shape=16,
                   alpha=opacity*0.3,
                   size=1,
      ...
      geom_point(data=data_mon_clim, 
                 aes(x=data_precip_PET_ups_mon, y=data_hws_obs_mon, color = "median hysteresis"), 
                 color="red",
                 size=4,
                 shape=16,
                 alpha=opacity) +
      geom_text_repel(data=data_mon_clim, 
                      aes(x=data_precip_PET_ups_mon, y=data_hws_obs_mon,
                          label=abbreviate(month.name, use.classes = FALSE, 
                                           minlength = 3, strict = TRUE)),
                      alpha=opacity ) +
      geom_segment(data=data_mon_clim,
                   aes(xend=c(tail(data_precip_PET_ups_mon, n=-1), NA), 
                       yend=c(tail(data_hws_obs_mon, n=-1), NA)),
      ...
      geom_point(data=data_mon_hws_max, 
                 aes(x=data_precip_PET_ups_mon, y=data_hws_obs_mon, color="enclosing maximum water elevation"), 
                 color="blue3",
                 size=4,
                 shape=16,
                 alpha=opacity*0.3) +
      ...
      geom_point(data=data_mon_hws_min, 
                 aes(x=data_precip_PET_ups_mon, y=data_hws_obs_mon, color="enclosing minimum water elevation"), 
                 color="deepskyblue3",
                 size=4,
                 shape=16,
                 alpha=opacity*0.5) +
      geom_segment(data=data_mon_hws_min,
                   ...
                   alpha = opacity*0.3,
                   size = 0.5 ) +
      xlab("precip - PET (mm/ month)") +
      ylab("observed water elevation (masl)") +
      labs(title = nameLakes[i], subtitle = paste(year_common_start, "-", year_common_end, sep = " ")) +
      scale_colour_manual(values = c("median hysteresis"="red", 
                                     'enclosing maximum water elevation'="blue3",
                                     'enclosing minimum water elevation'="deepskyblue3")) +
  ...
  if( i == 1 ){
    ...
    # Annotate legend
    xplot <- xplot +
      annotate("point", x = 100, y = 555, color="red", size=3) +
      annotate("point", x = 100, y = 553, color="blue3", size=3) +
      annotate("point", x = 100, y = 551, color="deepskyblue3", size=3) +
      annotate("text", x = 200, y = 555,   label = 'median hysteresis    ', size = 4) +
      annotate("text", x = 200, y = 553,   label = 'incl. max water level', size = 4) +
      annotate("text", x = 200, y = 551,   label = 'incl. min water level', size = 4)
  }
...
xlay <- matrix(c(1:nLakes,replicate((nRowPlot*nColPlot-nLakes), NA)), nRowPlot, nColPlot, byrow = TRUE)
...
# Output
pdf("regulation_hysteresis_connected_scatterplot.pdf", width = 5*nColPlot, height = 5*nRowPlot) # each subplot is 5x5 inches
