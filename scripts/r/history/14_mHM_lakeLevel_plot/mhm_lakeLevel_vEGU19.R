...
iopath = "/Users/shresthp/Nextcloud/Cloud/macbook/01_work/R/scripts/04_mHM_hydrograph_plot/02_mlm_meeting_26aug20/s2/"
...
jpeg(paste(iopath,fName,bName,".jpg",sep=""), width=9, height=4, units = "in", res = 300)

main <- ggplot(data, aes(x=date)) + geom_line(aes(y=data[,obsCol], color="observation"), size = 1, linetype ="dashed") + # The color statement needs to be inside aes for the legend to appear
        geom_line(aes(y=data[,simCol], color="mHM")) +
        ggtitle("Lake level : Tres Marias") +
        annotate("text", x = as.Date(paste(statPos,"-02-01",sep=""), format = "%Y-%m-%d"), y = qmax+7, cex = 5, label = paste("KGE", statKGE, sep = " ")) +
        annotate("text", x = as.Date(paste(statPos,"-02-01",sep=""), format = "%Y-%m-%d"), y = qmax+4,  cex = 5, label = paste("NSE", statNSE, sep = " ")) + 
        ylab(expression(paste("Daily level [masl]"))) + xlab("Year") +
        scale_colour_manual("", values = c("observation"="black", "mHM"="red")) +
