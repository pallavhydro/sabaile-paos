...
# convert 15' instantaneous obsevations to hourly values
...
for (ilead in leadStart:leadEnd) { # Lead time loop
  ...
  for (iens in ensStart:ensEnd) { # Ensemble members loop
    ...
    if (ilead == 1 && iens == 1){
      ...
      # Prepare the time origin
      tustr <- strsplit(tunits$value, "\\s+")
      tdstr <- strsplit(unlist(tustr)[3], "-")
      tmonth <- as.integer(unlist(tdstr)[2])
      tday <- as.integer(unlist(tdstr)[3])
      tyear <- as.integer(unlist(tdstr)[1])
      tfinal <- as.POSIXct(as.Date(nctime/24 - 2/24, origin=paste(tyear,tmonth,tday, sep = "-"))) # subtracting 2 hr
      # tchron <- as.Date(chron(nctime/24, origin=c(tmonth, tday, tyear))) # nctime (days)
      # tfinal <- as.POSIXct(tchron, tz = "GMT", origin=paste(tyear,tmonth,tday, sep = "-")) # nctime (hours)
    ...
    # convert to xts
    mhm_q_frcst <- xts(as.numeric(qsim), order.by = tfinal) # xts/ time series object created
